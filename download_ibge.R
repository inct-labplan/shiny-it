...
tasks <- list(
  ...
  list(
    name = "Regioes",
    url = "https://geoftp.ibge.gov.br/organizacao_do_territorio/malhas_territoriais/malhas_municipais/municipio_2024/Brasil/BR_UF_2024.zip",
    rename = c(id_uf = "CD_UF", nome_uf = "NM_UF")
  ),
  list(
    name = "MunicipiosRM",
...
for (task in tasks) {
  ...
  } else if (task$name == "Regioes") {
    cat("Agregando estados em Grandes Regiões do Brasil...\n")
    
    # Renomeia colunas iniciais da UF
    sf_data <- sf_data %>% rename(any_of(task$rename))
    
    # Cria os identificadores das Regiões com base no primeiro dígito do id_uf
    sf_data <- sf_data %>%
      mutate(
        identificador_unidade_territorial = stringr::str_sub(id_uf, 1, 1),
        nome_unidade_territorial = case_when(
          identificador_unidade_territorial == "1" ~ "Norte",
          identificador_unidade_territorial == "2" ~ "Nordeste",
          identificador_unidade_territorial == "3" ~ "Sudeste",
          identificador_unidade_territorial == "4" ~ "Sul",
          identificador_unidade_territorial == "5" ~ "Centro-Oeste"
        ),
        unidade_territorial = "Região"
      ) %>%
      # Agrupa pelas variáveis da região e dissolve os polígonos dos estados
      group_by(identificador_unidade_territorial, nome_unidade_territorial, unidade_territorial) %>%
