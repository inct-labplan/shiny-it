...
arquivos_yohana_vinculos <- list.files(dados_dir, pattern = "^vinculos_cidades_.*\\.xlsx$", full.names = TRUE)
...
all_data <- lapply(arquivos_yohana_vinculos, processar_planilha) %>%
  bind_rows()

# 6. Join com Dicionário e formatação final
vinculos_output <- all_data %>%
...
message("Total de registros: ", nrow(vinculos_output))
message("------------------------------------------")