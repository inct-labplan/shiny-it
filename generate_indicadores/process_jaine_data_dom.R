...
arquivos_jaine <- list.files(dados_dir, pattern = "^caracteristicas_domicilios_municipiosbr_.*\\.xlsx$", full.names = TRUE) #"^Dados_total_.*\\.xlsx$"
...
processar_planilha <- function(caminho) {
  ...
  cols_para_converter <- setdiff(
    names(data_filtered), 
    c("recorte_analise", "conteudo", "unidade_territorial", 
      "identificador_unidade_territorial", "nome_busca", 
      "id_uf", "id_muni", "id_rm")
  )
  data_filtered <- data_filtered %>%
    mutate(across(all_of(cols_para_converter), ~ as.numeric(as.character(.))))
  # Encontrar a posição da primeira variável (MSII)
  cols_to_pivot <- names(data_filtered)[which(names(data_filtered) == "tipodom_casa"):ncol(data_filtered)]
...
dom_output <- all_data %>%
  ...
  mutate(
    eixo = "Eixo 4", 
    projeto = "Características dos domicilios", 
...
message("Total de registros: ", nrow(dom_output))
message("------------------------------------------")
