...
dados_dir <- file.path("dados_yohana")
malhas_dir <- file.path("ibge_malhas")

arquivos_yohana <- list.files(dados_dir, pattern = "^Estabelecimentos_cidades_.*\\.xlsx$", full.names = TRUE)
dicionario_path <- file.path(dados_dir, "dicionario_ckan_estabelecimentos.xlsx")
...
processar_planilha <- function(caminho) {
  ...
  # Identifica colunas numéricas de indicadores a partir de "ALFACE"
  cols_to_pivot <- names(data_filtered)[which(names(data_filtered) == "baseado_em_ciencia"):ncol(data_filtered)]
...
all_data <- lapply(arquivos_yohana, processar_planilha) %>%
  bind_rows()

# 6. Join com Dicionário e formatação final
estabelecimentos_output <- all_data %>%
  left_join(dict, by = "variavel") %>%
  mutate(
    eixo = "Eixo 1", 
    projeto = "Estabelecimentos", 
...
message("Total de registros: ", nrow(estabelecimentos_output))
message("------------------------------------------")