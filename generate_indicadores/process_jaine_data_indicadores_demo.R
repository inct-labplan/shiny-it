...
arquivos_jaine <- list.files(dados_dir, pattern = "^indicadores_demograficos_municipiosbr_.*\\.xlsx$", full.names = TRUE)
...
processar_planilha <- function(caminho) {
  ...
  colunas_para_deletar <- c("variacao_abs_intercens_populacao", "taxa_cresc_intercensitario_rural",
                            "taxa_cresc_intercensitario_urbano", "taxa_cresc_intercensitario_total",
                            "densidade_demografica", "porte_pop_adapt", "porte_pop_ibge", "hierarq_urb_regic2018",
                            "num_hierarq_urb_regic2018_principais", "hierarq_urb_regic2018_principais")
  ...
  data_filtered <- data_filtered %>%
    # Converte todas as colunas que começam com "pop_"
    mutate(across(starts_with("pop_"), as.numeric))
  data_filtered <- data_filtered %>%
    # Converte todas as colunas que começam com "razao_"
    mutate(across(starts_with("razao_"), as.numeric))
  data_filtered <- data_filtered %>%
    # Converte todas as colunas que começam com "indice"
    mutate(across(starts_with("indice"), as.numeric))
  data_filtered <- data_filtered %>%
    # Converte todas as colunas que começam com "taxa_urbanizacao"
    mutate(across(starts_with("taxa_urbanizacao"), as.numeric))
  cols_to_pivot <- names(data_filtered)[which(names(data_filtered) == "pop_rural"):ncol(data_filtered)]
...
ind_demo <- all_data %>%
  ...
  mutate(
    eixo = "Eixo 4", 
    projeto = "Indicadores demográficos", 
...
message("Total de registros: ", nrow(ind_demo))
message("------------------------------------------")