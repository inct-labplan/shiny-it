...
diretorio_file <- "diretorio_ibge.parquet"
...
if (!file.exists(diretorio_file)) {
  stop("Erro: Arquivo ", diretorio_file, " não encontrado. Execute generate_indicadores/gen_indicadores.R primeiro.")
}

dados_indicadores <- arrow::read_parquet(parquet_file)
map_legend_data <- arrow::read_parquet(legend_file)
diretorio_ibge <- arrow::read_parquet(diretorio_file)
